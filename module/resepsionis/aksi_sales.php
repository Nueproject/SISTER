<?php
	...
	if(isset($_POST['tambahProductListQuotation'])){

		// data form
		$_SESSION['formDataQuotation'] = $_POST;
		...
		if(array_key_exists('id_product', $product)){
			$_SESSION['productListQuotation'][] = $product;
		}		
	...
	if(isset($_POST['removeProductListQuotation'])){

		//echo $_POST['removeProductList'];
		$_SESSION['formDataQuotation'] = $_POST;

		if(trim($_POST['removeProductListQuotation']) !== ''){
			...
			foreach ($_SESSION['productListQuotation'] as $input){
				...
				if($input['id_product'] == $_POST['removeProductListQuotation']){
					
					array_splice($_SESSION['productListQuotation'], $i, 1);
	...
	//ini belum bisa
	if(isset($_POST['tambahProductListUpdateQuotation'])){

		// data form
		$_SESSION['formDataUpdateQuotation'] = $_POST;
		
		// data product list
		$product = "select * from cera_product where id_product = '".$_POST['idProductListUpdateQuotation']."'";
		$product = mysqli_query($koneksi,$product);
		$product = mysqli_fetch_array($product);

		if(array_key_exists('id_product', $product)){
			$_SESSION['productListUpdateQuotation'][] = $product;
		}		
	...
	if(isset($_POST['removeProductListUpdateQuotation'])){
	...
	if(isset($_POST['saveQuotation'])){
		...
		$perusahaan = $_SESSION['formData']['perusahaan'];
	    ...
	    $simpanclient = mysqli_query($koneksi, "insert into cera_client (client_name, address_client, email_client, phone_client, perusahaan) values ('$nama', '$alamat', '$email', '$telp', '$perusahaan')") or die(mysqli_error($koneksi));

	    $idclient = mysqli_query($koneksi, "select max(id_client) AS idclient from cera_client");
	    $id_cl = $idclient->fetch_assoc();
	    $queryclient = mysqli_query($koneksi, "select * from cera_client where id_client = '$id_cl'");
	    $clientData = mysqli_fetch_array($queryclient);
...
if(isset($_POST['POToInvoice'])){
		...
		$tgl_skrg = date("Y-m-d");
		$nama = $_SESSION['formDataUpdate']['nama_client'];
		$nomor = $_SESSION['formDataUpdate']['quotationNo'];
		$alamat = $_SESSION['formDataUpdate']['alamat'];
		$email = $_SESSION['formDataUpdate']['email'];
		$telp = $_SESSION['formDataUpdate']['phone_client'];
				...
				sales_nama_client='".$nama."', 
				sales_alamat_client='".$alamat."', 
				sales_email_client='".$email."', 
				sales_telp_client='".$telp."',
				updated_at='$tgl_skrg',
				sales_id_status='3'
	    ...
	    $querya = mysqli_query($koneksi, "select * from cera_sales where sales_id = '".$_POST['sales_id']."'");
	    $invoiceData = mysqli_fetch_array($querya);
...
if(isset($_POST['SaveInvoice'])){
		...
		$simpanclient = mysqli_query($koneksi, "insert into cera_client (client_name, perusahaan, address_client, email_client, phone_client) values ('$nama', '$perusahaan', '$alamat','$email', '$telp' )") or die(mysqli_error($koneksi));
...
if(isset($_POST['InvToKwitansi'])){
		...
		$jumlahbayar = $_SESSION['formDataUpdate']['totalbayar'];
				...
				sales_email_client='".$_POST['email']."', 
				sales_telp_client='".$_POST['phone_client']."',
				sales_guna_bayar='".$gunaBayar."',
				sales_jumlah_bayar='".$jumlahbayar."',
		...
		foreach($_POST['id'] as $d){
            ...
            mysqli_query($koneksi, "update cera_sales SET 
            		sales_jumlah_bayar='".$_POST['totalbayar'][$i]."' 
	...
	if(isset($_POST['editPO'])){
		...
		foreach ($_SESSION['productListUpdate'] as $input) {
            ...
            mysqli_query($koneksi, "insert into cera_sales_item 
				(si_id,si_sales_id, si_item_name, si_product_id, si_item_qty, si_item_price, si_item_deskripsi, si_item_size, si_item_gambar) 
				values ('','$idSales', '".$input['product_name']."', '".$input['id_product']."', '".$_POST['qtynew'][$i]."', '".$_POST['harganew'][$i]."', '".$_POST['descnew'][$i]."', '".$_POST['sizenew'][$i]."', '".$_POST['gambar'][$i]."')") or die(mysqli_error());
			$i++;
		}
